...
public class FireworkExplosionPlayer {
	...
	static {
		try {
			...
			packetPlayOutEntityStatus = getMCClass("PacketPlayOutEntityStatus")
					.getConstructor(getMCClass("Entity"), byte.class);
			// get the getHandle method for players
			getEntityHandle = getCraftClass("entity.CraftPlayer").getMethod(
					"getHandle");
			// get the field for the playerconnection
			getPlayerConnection = getMCClass("EntityPlayer").getDeclaredField(
					"playerConnection");
			...
			getFireworkHandle = getCraftClass("entity.CraftEntity").getMethod(
					"getHandle");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/*
	 * playToPlayer(Player p, Location loc, FireworkEffect fe) Player p - the
	 * only player who sees the explosion. Location loc - the location where the
	 * explosion happends FireworkEffect fe - the fireworkeffect, example:
	...
	/*
	 * playToLocation(Location loc, FireworkEffect fe) Location loc - the
	 * location where the explosion happends FireworkEffect fe - the
	 * fireworkeffect, example:
	 * FireworkEffect.builder().with(Type.BALL).withColor(Color.RED).build()
	 */
	...
	private static Object makePacket(Location loc, FireworkEffect fe) {
		try {
			...
			firework.remove();
			return packetPlayOutEntityStatus.newInstance(nms_firework,
					(byte) 17);
		} catch (Exception e) {
			e.printStackTrace();
		}
